...
;;; Availability
;;;--------------
;;; Anonymous FTP from think.com in the completion subdirectory
;;;

;;;---------------------------------------------------------------------------
...
;;;-----------------------------------------------
;;;
;;;  Should run on 18.49, 18.52, 18.55, 18.56 and 19.0
...
;;;      terminals only !)  [I believe this has been done by
;;;                          eero@media-lab.media.mit.edu] 
;;;   Add doc. to texinfo
;;;
;;;
;;;-----------------------------------------------
...
;;; Known Bugs
;;;-----------------------------------------------
;;;  The only known bugs are on some Motorola RISC platforms running Sys V
;;;  Unix.  The effects follow.  I do not understand the causes but since
;;;  they've only occured on those platforms I assume they are Emacs
;;;  implementation problems. 
;;;      1) def-completion-wrapper doesn't work correctly on autoloaded
;;;         functions.  Workaround: load the autoloaded
;;;         functions before loading completion.
;;;      2) Pressing a right parenthesis while the emacs idle process is
;;;         running causes an annoying "select error".  Workaround: don't
;;;         use the idle process (set *completion-idle-program* to nil)
;;;
;;;-----------------------------------------------
;;; Change Log
;;;-----------------------------------------------
;;; From version 10.2 to 10.3
;;;  - Renamed meta-/ to complete as well as meta-return (Brewster sez RMS 
;;;  suggested it.) 
;;;  - fixed call to "wakeup" to not pass the "-n" argument
;;;
;;; From version 10.1 to 10.2
;;;  - Added *completion-idle-program* variable 
;;;     This defaults to "wakeup" for 18.56 and beyond.  "loadst" in
;;;     earlier versions.
;;;  - Added *completion-find-file-hook-c-major-modes* and
;;;          *completion-find-file-hook-lisp-major-modes* 
;;;  - Fixed completion-backup-filename to keep the file length less 
;;;    than 14 chars. for Sys V compatibility. (fixed)
;;;
;;; From version 10 to 10.1
;;;  - Made erasure of last completion (via c-w or repeated <complete> key 
;;;    pressing) use relative point position so it would work on top of a 
;;;    shell buffer that was continually printing
;;;  - Added error trap around call to init-cmpl-emacs-idle-process inside
;;;    of initialize-completions.  There is currently some bizarre bug that
;;;    causes it to get a (wrong-type-argument stringp) error inside of
;;;    start-process.  The bug is extremenly context sensitive and I'd
;;;    appreciate any help tracking it down if you can reproduce it.
;;;
...
(defvar *completion-find-file-hook-c-major-modes* '(c-mode)
  "*A list of major mode symbols for which cmpl-find-file-hook and
add-completions-from-file should use the c parser.")

(defvar *completion-idle-program* (if (or (>= cmpl-emacs-major-version 19)
					  (and (= cmpl-emacs-major-version 18)
					       (> cmpl-emacs-minor-version 55)))
				      '("wakeup") '("loadst" "-n"))
  "*The list of the idling program name and standard arguments or NIL if
you don't want any idling program.  This must be set before
initialize-completions is called.   The program is usually the one that is
shipped with emacs that display-time uses. It is given one additional
argument which is the delay in seconds.") 
...
(defun cmpl19-sit-for (amount)
  (if (and (>= cmpl-emacs-major-version 19) (= amount 0))
      (sit-for 1 t)
      (sit-for amount)))
...
(defun init-cmpl-emacs-idle-process ()
  ...
  (let ((live (and cmpl-emacs-idle-process
    ...
    (when *completion-idle-program*
      (setq cmpl-emacs-idle-process
	    (apply 'start-process "cmpl-emacs-idle" nil
		   (append *completion-idle-program*
			   (list (int-to-string cmpl-emacs-idle-interval)))))
...
(defvar cmpl-last-insert-length nil) ;; negative if point left at start
...
;; Maybe necessary if using 8-bit patches
;;(define-key esc-map "\r" 'complete)

;;; Redefine meta-/ since completion does it one better.
...
(defun completion-backup-filename (filename)
  ;; Make sure the total length is less than 14 chars (Sys V restriction)
